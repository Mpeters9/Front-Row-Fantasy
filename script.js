document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchPlayers() {
        try {
            ...
            const data = await response.json();
            return Object.values(data).filter(player => player.fantasy_positions && player.team).slice(0, 50); // Limit for performance
        } catch (error) {
    ...
    async function fetchADP(leagueType, teams = 12) {
        const apikey = 'YOUR_FANTASY_NERDS_API_KEY'; // Replace with your API key
        const format = leagueType === 'superflex' ? 'superflex' : scoringSelect.value;
    ...
    async function fetchTransactions(leagueId, round = 'all') {
        const url = `https://api.sleeper.app/v1/league/${leagueId}/transactions/${round}`;
        try {
            const response = await fetch(url);
            const data = await response.json();
            return data.filter(tx => tx.type === 'trade').slice(0, 5); // Limit to recent trades
        } catch (error) {
    ...
    async function populatePlayers() {
        ...
        const options = players.map(player => {
            const adp = adpData.find(p => p.name === player.full_name) || { redraftValue: 10, dynastyValue: 10 };
            return `<option value="${player.player_id}" data-position="${player.fantasy_positions[0]}" data-redraft="${adp.redraftValue}" data-dynasty="${adp.dynastyValue}">${player.full_name} (${player.fantasy_positions[0]})</option>`;
        }).join('');
    ...
    async function displayRecentTrades(leagueId) {
        ...
        recentTrades.innerHTML = transactions.length ? transactions.map(tx => {
            const players = Object.entries(tx.adds || {}).map(([playerId, rosterId]) => {
    ...
    syncLeagueBtn.addEventListener('click', async () => {
        ...
        if (leagueId) {
            ...
            tradeResult.textContent = 'Please enter a valid Sleeper League ID.';
        }
    });
    ...
    leagueTypeSelect.addEventListener('change', populatePlayers);
    ...
    async function fetchMatchupTeams() {
        try {
            const response = await fetch('https://api.sleeper.app/v1/league/YOUR_LEAGUE_ID/rosters');
            const data = await response.json();
            if (data && data.length) {
