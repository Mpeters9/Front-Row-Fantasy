document.addEventListener('DOMContentLoaded', () => {
    ...
    const fallbackPlayers = [
        { name: "Christian McCaffrey", position: "RB", team: "SF", bye: 7, ppr_rank: 1, half_ppr_rank: 1, standard_rank: 1, points: 22.5 },
        { name: "CeeDee Lamb", position: "WR", team: "DAL", bye: 7, ppr_rank: 6, half_ppr_rank: 6, standard_rank: 6, points: 20.1 },
        { name: "Breece Hall", position: "RB", team: "NYJ", bye: 6, ppr_rank: 3, half_ppr_rank: 3, standard_rank: 3, points: 19.8 },
        { name: "Justin Jefferson", position: "WR", team: "MIN", bye: 6, ppr_rank: 4, half_ppr_rank: 4, standard_rank: 4, points: 20.3 },
        { name: "Patrick Mahomes", position: "QB", team: "KC", bye: 6, ppr_rank: 10, half_ppr_rank: 10, standard_rank: 10, points: 23.1 }
    ];
    ...
    const leagueSizeSelect = $('leagueSize');
    const scoringTypeSelect = $('scoringType');
    const draftPickInput = $('draftPick');
    const draftPickValue = $('draftPickValue');
    const generateDraftButton = $('generateLineup');
    const buildResultDraft = $('build-result');
    const benchSizeSelect = $('benchSize');
    ...
    if (scoringTypeSelect) scoringTypeSelect.value = 'ppr'; // Default to PPR
    ...
    // --- Fetch Sleeper Players for Trade Analyzer ---
    async function fetchSleeperPlayers() {
        ...
        const player1Search = $('player1-search');
        const player1Autocomplete = $('player1-autocomplete');
        const player2Search = $('player2-search');
        const player2Autocomplete = $('player2-autocomplete');
    ...
    function analyzeTrade() {
        ...
        $('trade-fairness').innerHTML = `<span class="px-3 py-1 rounded ${color} text-white">${emoji} ${fairnessText}</span>`;

        const ctx = $('tradeValueChart')?.getContext('2d');
        if (ctx) {
    ...
    function clearAll() {
        ...
        renderTeam('team1-players', team1, 'team1');
        renderTeam('team2-players', team2, 'team2');
        $('trade-fairness').innerHTML = '';
        $('ai-advice').textContent = '';
        if (window.tradeChart) window.tradeChart.destroy();
    }
    ...
    let allPlayers = [];
    let currentScoring = 'ppr'; // Default to PPR
    ...
    function getScoringType() {
        return scoringTypeSelect?.value || 'ppr';
    }
    ...
    function loadPlayersAndInitTools(scoringType) {
        const jsonFile = scoringFiles[scoringType] || 'PPR.json';
        showLoadingSpinner(true);
        const cachedData = localStorage.getItem(`players_${scoringType}`);
        if (cachedData) {
        ...
        fetch(jsonFile)
            ...
            .then(data => {
                ...
                localStorage.setItem(`players_${scoringType}`, JSON.stringify(allPlayers));
                buildPlayerTable();
                populatePlayerList();
            })
            .catch(error => {
                console.error('Error loading players:', error);
                allPlayers = fallbackPlayers.map(player => ({
                    Player: player.name,
                    POS: player.position,
                    Team: player.team,
                    Bye: player.bye,
                    AVG: scoringType === 'ppr' ? player.ppr_rank :
                         scoringType === 'half' ? player.half_ppr_rank : player.standard_rank,
                    Points: player.points
                }));
                buildPlayerTable();
                populatePlayerList();
                alert(`Failed to load ${jsonFile}. Using fallback data.`);
            })
    ...
    const draftLineupPreset = $('draftLineupPreset');
    if (draftLineupPreset) {
        ...
        draftLineupPreset.addEventListener('change', function () {
            const draftQbCount = $('draftQbCount');
            const draftSfCount = $('draftSfCount');
            const draftRbCount = $('draftRbCount');
            const draftWrCount = $('draftWrCount');
            const draftTeCount = $('draftTeCount');
            const draftFlexCount = $('draftFlexCount');
            const draftDstCount = $('draftDstCount');
            const draftKCount = $('draftKCount');
    ...
    const generateLineupBtn = $('generateLineup');
    if (generateLineupBtn) {
        generateLineupBtn.addEventListener('click', function () {
            if (!leagueSizeSelect || !draftPickInput || !scoringTypeSelect || !benchSizeSelect) {
                alert('Required form elements are missing. Please check the page setup.');
                return;
            }
            const leagueSize = parseInt(leagueSizeSelect.value, 10);
            const draftPick = parseInt(draftPickInput.value, 10);
            const scoringType = getScoringType();
            const benchSize = parseInt(benchSizeSelect.value, 10) || 6;
            ...
            loadPlayersAndInitTools(scoringType);
            simulateDraft({
                allPlayers,
                leagueSize,
                draftPick,
    ...
    // --- Initial Load ---
    if (scoringTypeSelect) scoringTypeSelect.value = 'ppr';
    loadPlayersAndInitTools(currentScoring);
});
