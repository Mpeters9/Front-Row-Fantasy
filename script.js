    ...
    const App = {
        ...
        initAiChat() {
            ...
            const addMessage = (message, sender) => {
                ...
                messageElement.innerHTML = message; // Use innerHTML to render potential HTML from AI
        ...
        populateFilterOptions(controls) {
            ...
            tiers.forEach(tier => {
                if(![...controls.tierFilter.options].some(o => o.value == tier)) controls.tierFilter.add(new Option(`Tier ${tier}`, tier));
            });
            teams.forEach(team => {
                if(![...controls.teamFilter.options].some(o => o.value == team)) controls.teamFilter.add(new Option(team, team));
            });
        },
        ...
        loadArticleContent() {
            ...
            if (!container) return;
            container.innerHTML = `<h2>This is a Placeholder Article Title</h2><p>This page is a template for individual articles. In a full build, clicking an article on the main articles page would lead here, and the content for that specific article would be loaded. For now, we are focusing on the AI-powered "Briefing Room" on the main articles page.</p><p>This approach allows for a flexible content management system where new articles can be added dynamically without needing to create new HTML files for each one.</p>`;
        },
        initWaiverWirePage() {
